...
const Stype typeVoid    = &(typeConst[0]);
const Stype typeInteger = &(typeConst[1]);
const Stype typeBoolean = &(typeConst[2]);
const Stype typeChar    = &(typeConst[3]);
const Stype typeReal    = &(typeConst[4]);
...
SymbolEntry * newVariable (const char * name, Stype type)
{
...
SymbolEntry * newConstant (const char * name, Stype type, ...)
{
    SymbolEntry * e;
    va_list ap;
...
SymbolEntry * newParameter (const char * name, Stype type,
...
void endFunctionHeader (SymbolEntry * f, Stype type)
{
    if (f->entryType != ENTRY_FUNCTION)
...
SymbolEntry * newTemporary (Stype type)
{
    char buffer[10];
    SymbolEntry * e;
...
SymbolEntry * lookupEntry (const char * name, LookupType type, bool err)
{
    ...
    if (err)
        error("Unknown identifier: %s, mmmmmmmmmmmmmmmmmm", name);
    return NULL;
}

Stype typeArray (RepInteger size, Stype refType)
{
    Stype n = (Stype) new(sizeof(struct Type_tag));
...
Stype typePointer (Stype refType)
{
    Stype n = (Stype) new(sizeof(struct Type_tag));
...
unsigned int sizeOfType (Stype type)
{
    switch (type->kind) {
        case TYPE_VOID:
            internal("Stype void has no size");
            break;
        case TYPE_INTEGER:
        case TYPE_IARRAY:
        case TYPE_POINTER:
...
bool equalType (Stype type1, Stype type2)
{
